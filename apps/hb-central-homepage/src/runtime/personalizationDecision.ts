export type PersonalizationDecisionState = "deferred";

export interface PersonalizedLowerZoneRequirement {
  id: string;
  description: string;
}

export interface PersonalizationDecisionContract {
  state: PersonalizationDecisionState;
  rationale: string;
  requirements: readonly PersonalizedLowerZoneRequirement[];
  placeholderTitle: string;
  placeholderBody: string;
}

export interface GlobalUtilityLinkItem {
  id: string;
  label: string;
  href: string;
  external?: boolean;
...
export const PERSONALIZATION_DECISION: PersonalizationDecisionContract = {
  state: "deferred",
  rationale:
    "Current source adapters do not provide stable, per-user signal quality needed for meaningful personalization.",
  requirements: [
    {
  ...
  placeholderBody:
    "A governed seam is active until source quality supports stable, high-value personalization.",
};

export const GLOBAL_UTILITY_LINKS: readonly GlobalUtilityLinkItem[] = [
