import {
  HOMEPAGE_DYNAMIC_DATA_ENABLED,
  HOMEPAGE_SURFACE_AUTHORING_CONFIG,
  adaptCompanyPulseSource,
  adaptFeaturedProjectsSource,
  adaptGlobalUtilityLinks,
  adaptNewsRecognitionSource,
  adaptPeopleMomentsSource,
  adaptPersonalizationDecision,
  adaptQuickActionsSource,
} from "./homepageAuthoringConfig";

describe("homepageAuthoringConfig", () => {
  it("defines expected source modes and defaults per surface", () => {
  ...
  it("supports adapter-level source switching and max-item overrides", () => {
    const featured = adaptFeaturedProjectsSource(
      {
        items: [
    ...
    const people = adaptPeopleMomentsSource(
      {
        items: [
  ...
  it("normalizes missing/weak fields and surfaces empty seam states intentionally", () => {
    const actions = adaptQuickActionsSource(
      {
    ...
    const pulse = adaptCompanyPulseSource({ items: [] });
    expect(pulse.state).toBe("empty");

    const mosaic = adaptNewsRecognitionSource({ news: [], recognition: [], spotlight: [] });
    expect(mosaic.state).toBe("empty");
  });

  it("returns decision seams for personalization and footer utility", () => {
    const personalization = adaptPersonalizationDecision();
    expect(personalization.sourceMode).toBe("deferred");
    expect(personalization.decision.requirements.length).toBeGreaterThan(0);

    const utility = adaptGlobalUtilityLinks();
