...
function normalizeItem(item, maxLabelLength) {
    return __assign(__assign({}, item), { label: normalizeLabel(item.label, maxLabelLength) });
}
function normalizeQuickActions(source, options) {
    ...
    if (source.groups && source.groups.length > 0) {
        return source.groups.map(function (group) { return (__assign(__assign({}, group), { items: group.items
